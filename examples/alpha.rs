...
fn main() {
    ...
    while ui.window.open() {
        ui.frame(|ui| {
            ...
            ui.rect(rect().x(0).y(0).width(w).height(h).bg(rgb(24, 24, 30)));
            ui.rect(rect().x(w / 2).y(0).width(w - w / 2).height(h).bg(rgb(228, 228, 235)));
            ...
            ui.text("opaque white", text().x(70).y(576).fg(white()).font_size(22));
            ui.text(
                "40% white",
                text().x(w / 2 + 30).y(576).fg(with_alpha(white(), 102)).font_size(22),
            );
        });
    }
}
