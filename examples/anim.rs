...
fn main() {
    ...
    while ui.window.open() {
        ui.frame(|ui| {
            ...
            ui.flow_down(flow().pad(12).gap(16), |ui| {
                ui.text("OK — distinct slots in a scope", text().fg(white()));
                let w = ui.animate_f32(if open { 240.0 } else { 80.0 }, 0.35, Ease::OutCubic);
                let bar = ui.text(
                    if open { "close" } else { "open" },
                    text().width(w as i32).height(36).bg(hex("#3b82f6")).fg(white()),
                );
                if bar.clicked {
                    open = !open;
                }

                ui.text("OK — helper calls (scope slots)", text().fg(hex("#4ade80")));
                ui.flow_right(flow().gap(8), |ui| {
                    ...
                    hover_a = ui.text("A", text().width(80).height(36).bg(ca).fg(white())).hovered;
                    hover_b = ui.text("B", text().width(80).height(36).bg(cb).fg(white())).hovered;
                });

                ui.text("OK — one animate per nested scope", text().fg(hex("#4ade80")));
                for i in 0..3 {
                ...
                ui.text("OK — with_id (stable keys)", text().fg(hex("#4ade80")));
                ui.flow_right(flow().gap(8), |ui| {
                    if ui.text("+", text().width(28).height(28).bg(hex("#3f3f46")).fg(white())).clicked {
                        items.insert(0, (next_id, false));
                        next_id += 1;
                    }
                    if ui.text("rot", text().width(40).height(28).bg(hex("#3f3f46")).fg(white())).clicked {
                        items.rotate_left(1);
                    }
                    for (id, on) in items.iter_mut() {
                        ui.with_id(*id, |ui| {
                            let c = fade(ui, if *on { hex("#3b82f6") } else { hex("#52525b") });
                            if ui.text(format!("{id}"), text().width(48).height(32).bg(c).fg(white())).clicked {
                                *on = !*on;
                            }
                        });
                    }
